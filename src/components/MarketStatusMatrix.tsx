...
function StatusMatrixTooltip({ tooltip }: { tooltip: MatrixTooltipState }) {
  if (tooltip.kind === 'market') {
    return (
      <div
        className="pointer-events-none fixed z-[100] -translate-x-1/2 rounded-lg border border-gray-200 bg-white px-2.5 py-2 shadow-lg"
        style={{ left: tooltip.x, top: tooltip.y + 8 }}
      >
        <p className="text-[11px] font-semibold whitespace-nowrap text-gray-900">
          {tooltip.marketLabel}
        </p>
        <p className="mt-0.5 text-[10px] whitespace-nowrap text-gray-500">
          {tooltip.abbrev}
        </p>
      </div>
    )
  ...
  const dot = CELL_STATUS_DOT[tooltip.status]

  return (
    <div
      className="pointer-events-none fixed z-[100] -translate-x-1/2 -translate-y-[calc(100%+8px)] rounded-lg border border-gray-200 bg-white px-2.5 py-2 shadow-lg"
    ...
    >
      ...
      {tooltip.playerName && tooltip.marketLabel ? (
        <p className="mt-1 text-[11px] whitespace-nowrap text-gray-700">
          {tooltip.playerName} · {tooltip.marketLabel}
        </p>
      ) : null}
      <p className="mt-0.5 text-[10px] whitespace-nowrap text-gray-500">
  ...
  return (
    <>
      <section className="overflow-hidden rounded-2xl border border-[#e5e7eb] bg-white">
        ...
        <div className="max-h-[min(28rem,calc(100dvh-16rem))] overflow-auto">
          <table className="w-full border-collapse text-left">
            <thead className="sticky top-0 z-20 bg-[#f9fafb]">
              <tr>
                <th className="sticky left-0 z-30 min-w-[9rem] border-b border-[#e5e7eb] bg-[#f9fafb] px-3 py-2 text-xs font-medium text-gray-500">
                  Player
                </th>
                {MARKET_COLUMNS.map((column) => (
                  <th
                    key={column.key}
                    className="border-b border-[#e5e7eb] px-1 py-2 text-center text-[10px] font-medium text-gray-500"
                  >
                    <span
            ...
            <tbody>
              {matrix.map(({ player, cells }) => (
                <tr key={player.id} className="border-b border-[#f3f4f6] last:border-0">
                  <td className="sticky left-0 z-10 bg-white px-3 py-1.5">
                    <div className="flex min-w-0 items-center gap-1.5">
                      ...
                      <TruncatedText className="text-xs text-gray-800">
                        {player.name}
                      </TruncatedText>
                    </div>
                  </td>
                  {cells.map((cell) => {
                    const dot = CELL_STATUS_DOT[cell.status]
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-[#e5e7eb] px-4 py-2.5">
          {legendStatuses.map((status) => (
            <span
              key={status}
              className="inline-flex cursor-default items-center gap-1.5 text-[11px] text-gray-600"
              onMouseEnter={(event) =>
                showStatusTooltip(
                  event,
                  {
            ...
            >
              <span
                ...
                style={{ backgroundColor: CELL_STATUS_DOT[status].color }}
              />
              {CELL_STATUS_DOT[status].label}
            </span>
          ))}
        </div>
      </section>

      {tooltip ? <StatusMatrixTooltip tooltip={tooltip} /> : null}
    </>
  )
}
